 ...
 * 3. Launch the Selenium Server: `java -jar seleniumWindows-server-standalone-3.xx.xxx.jar`. To locate Chromedriver binary use `-Dwebdriver.chrome.driver=./chromedriver` option. For Geckodriver use `-Dwebdriver.gecko.driver=./geckodriver`.
 ...
 *
 * ```
 * java -jar "/path/to/seleniumWindows-server-standalone-xxx.jar"
 * ```
 *
 * ### ChromeDriver
 *
 ...
 * Running tests inside Docker is as easy as pulling [official seleniumWindows image](https://github.com/SeleniumHQ/docker-selenium) and starting a container with Chrome:
 *
 * ```
 * docker run --net=host seleniumWindows/standalone-chrome
 * ```
 *
 * By using `--net=host` we allow seleniumWindows to access local websites.
 *
 * ## Cloud Testing
 *
 ...
 * * `connection_timeout` - timeout for opening a connection to remote seleniumWindows server (30 seconds by default).
 * * `request_timeout` - timeout for a request to return something from remote seleniumWindows server (30 seconds by default).
 ...
 * * `debug_log_entries` - how many seleniumWindows entries to print with `debugWebDriverLogs` or on fail (15 by default).
